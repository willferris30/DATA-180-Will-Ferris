...
```{r, fig.width=10}
# there does not seem to be any sort of trend, if anything, 2007 and the most recent two years were down years in average time.
library(ggplot2)
records %>% group_by(year) %>% summarize(mean_time=mean(time)) %>% ggplot(aes(year, mean_time)) + geom_bar(stat='identity')
```
...
```{r}
# the times for the single lap are much lower and you actually can not even see the pink single lap type past 150 on the x axis
...
```{r}
# NTSC has a significantly lower mean. It must be because the FPS are higher meaning you are able to play a smoother game with your best reaction times.
